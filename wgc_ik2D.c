...
int main() {
  ...
  Y0 = -L0/2;
  Z0 = -L0/2; 
  G.z = 50.; //'G' has an attribute 'y' to specify direction
...
event amplitude (t += 0.005; t <= 1) {
  ...
  char nameposfac[100];
  sprintf (nameposfac, "Kakade/wgc_2d/data1607/2D/3/datafacqcc-%g.txt",t);
  ...
  /* char namepos[100]; */
  /* sprintf (namepos, "Kakade/wgc_2d/data1607/2D/data-%g.txt",t); */
  /* FILE * fpos = fopen (namepos, "w"); */
  /* (all, fpos, N); //Does not work in 3D */
  /* fflush (fpos); */
  //fprintf (stderr, h[]);
...
event acceleration (i++)
{
  ...
  double b = 0;     //Position of Pressure
  coord G1;
  G1.z = -G.z;     //Direction of Applied
 if (phi.i)
     positionbumpstat2D(f, phi, a, Po, b, G1, Z, add = true);
     else {
     phi = new scalar;
     positionbumpstat2D(f, phi, a, Po, b, G1, Z, add = false);
     f.phi = phi;
    }
}
