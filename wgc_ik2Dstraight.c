...
event amplitude (t += 0.005; t <= 0.05) {
  scalar pos[];
  vector h[];
...
{
  scalar phi = f.phi;
  double a = 0.01;     //Width of Gaussian Distributed Force
  double Po = -200; //Magnitude of Force Applied
  double s = 2;     //Angular Velocity
  double b = 4;    //Radius of Rotation
  coord G1;
...
event adapt(i++)
{
  adapt_wavelet ({f}, (double []){1e-4}, maxlevel = 8, minlevel = 6);
  
  scalar l[];
  foreach()
    l[] = level;
  static FILE * fp = fopen ("gridrefine.ppm", "w");
